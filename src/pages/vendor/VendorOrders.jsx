...
import { FaEye, FaBox, FaTrash } from 'react-icons/fa';
import { useDispatch, useSelector } from 'react-redux';
import { fetchVendorOrders, updateOrder, resetUpdateStatus, clearErrors, deleteOrder } from '../../features/orders/orderSlice';
...
const VendorOrders = () => {
    ...
    const { vendorOrders: orders, loading, error, updateLoading, isUpdated } = useSelector((state) => state.orders);
    ...
    const updateStatusHandler = (id, status) => {
        if (status === 'Shipped') {
            Swal.fire({
                title: 'Delivery Coordination Checklist',
                html: `
                    <div style="text-align: left;">
                        <p>Please confirm the following before marking as Shipped:</p>
                        <ul style="list-style-type: none; padding: 0;">
                            <li><input type="checkbox" id="check1"> <label for="check1">Products packed securely</label></li>
                            <li><input type="checkbox" id="check2"> <label for="check2">Quality check passed</label></li>
                            <li><input type="checkbox" id="check3"> <label for="check3">Delivery service coordinated</label></li>
                            <li><input type="checkbox" id="check4"> <label for="check4">Invoice included</label></li>
                        </ul>
                    </div>
                `,
                icon: 'info',
                ...
                preConfirm: () => {
                    const check1 = Swal.getPopup().querySelector('#check1').checked;
                    const check2 = Swal.getPopup().querySelector('#check2').checked;
                    const check3 = Swal.getPopup().querySelector('#check3').checked;
                    const check4 = Swal.getPopup().querySelector('#check4').checked;

                    if (!check1 || !check2 || !check3 || !check4) {
                        Swal.showValidationMessage('Please complete all checklist items');
                    }
                ...
                if (result.isConfirmed) {
                    ...
                    // Reset selection if cancelled (optional, might require local state management for the select input to revert visually)
                    // For now, the dropdown might show 'Shipped' but the actual state won't update until refresh or successful dispatch.
                    // A better way would be handle local state for the select value, but this is a good start.
                }
            });
        } else {
            dispatch(updateOrder({ id, status }));
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="flex items-center justify-center space-x-2 w-full">
                    ...
                    <button onClick={() => deleteHandler(row._id)} className="text-red-600 hover:text-red-900 p-1" title="Delete Order">
                        <FaTrash size={18} />
                    </button>
                </div>
            ),
        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            <div className="flex-1 overflow-auto relative">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-gray-800 mb-4 md:mb-0">Vendor Orders</h1>
                    ...
                    <DataTable
                        ...
                        noDataComponent={
                            <div className="p-12 text-center">
                                ...
                                <p className="mt-2 text-gray-500">You haven't received any orders matching your criteria yet.</p>
                            </div>
                        }
                    />
                </div>
